...
#+BEGIN_SRC emacs-lisp
  (use-package org
    ...
    (setq org-latex-to-mathml-jar-file "~/Downloads/mathtoweb.jar"))
...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package whitespace
    ...
    :hook ((prog-mode sgml-mode org-mode text-mode) . whitespace-mode))
...
#+BEGIN_SRC emacs-lisp
    ...
    (setq exwm-input-global-keys
          (mapcar
           ...
           `(("s-h" . windmove-left) ; basic movement keys
             ...
             ,@(mapcar (lambda (x) ; workspace stuff
                         (cons
                          (format "s-%s" x)
                          (lambda ()
                            (interactive)
                            (let ((count (exwm-workspace--count)))
                              (exwm-workspace-switch-create x)
                              ;; When a workspace is created, split horizontally
                              ;; to compensate for two monitors. The entire frame
                              ;; is stretched over 2*1366 pixels otherwise.
                              (when (>= x count)
  ...
  (defun exwm-init-actions ()
    ...
    (split-window-right) ; initial workspace 0 split for two monitors
    (setq exwm-enabled t))
